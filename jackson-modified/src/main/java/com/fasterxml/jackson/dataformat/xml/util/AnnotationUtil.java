...
public class AnnotationUtil
{
    ...
    public static String findPrefixnameAnnotation(AnnotationIntrospector ai,
                                                 AnnotatedMember prop)
    {
        for (AnnotationIntrospector intr : ai.allIntrospectors()) {
            if (intr instanceof XmlAnnotationIntrospector) {
                String pr = ((XmlAnnotationIntrospector) intr).findPrefixname(prop);
                if (pr != null) {
                    return pr;
                }
            } else  if (intr instanceof JaxbAnnotationIntrospector) {
                String pr = ((JaxbAnnotationIntrospector) intr).findPrefixname(prop);
                if (pr != null) {
                    return pr;
                }
            }
